...
function getRectangleString(width, height) {
    ...
    var middleRepeat = middle.repeat(height - 2);
    var bottom = '└' + '─'.repeat(width - 2) + '┘\n'
    return top + middleRepeat + bottom;
}
...
function encodeToRot13(str) {
    var symbols = 'NOPQRSTUVWXYZABCDEFGHIJKLM[\]^_` nopqrstuvwxyzabcdefghijklm';
    var res = [str.length];
    let const1 = 65;
    let const2 = 122;
    ...
    for (var i = 0; i < str.length; i++) {
        ...
        if (charCode < const1 || charCode > const2) {
            res[i] = str[i];
        }
        else {
            res[i] = symbols[charCode - const1];
        }
    }
    return res.join("");
}
